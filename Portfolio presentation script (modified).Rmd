...
dataset <- drop_read_csv("portfolio/hotels.csv")
...
dataset_national_index_complete <- drop_read_csv("portfolio/dataset_national_index_csv.csv")
...
dataset_donations <- drop_read_csv("portfolio/dataset_main_donations_portf.csv")
...
* **Reporting**
<br>
Easy to read visualisations that help Revenue Managers make decisions about the pricing.
...
* **Segmentation of donors according to their Length, Recency, Frequency, Monetary, Periodicity of donations.**
<br>
...
* **Fitting a model that predicts future donor engagement.**
<br>
...
Five segments were identified, each with their own transactional patterns. A characterisation was performed after futher analysis of the clusters. 

---

```{r ML model stats, fig.align="center", echo = FALSE, warning = FALSE, error = FALSE, message = FALSE}

ObjectTableML <- read_csv("~/Google Drive/Thesis/Images/Table2 (csv).csv")
...
A classification model was performed, gathering concepts from Churn Prediction and Lifetime Value models, adapting them to a charity context as well as the problem to solve. 

* *The research document describing the framework is expected to be published*
...
<br>
As operational tools or managerial information systems, dashboards can inform the business about KPI's performance. To see an example access [this link](https://ftoresh.shinyapps.io/portfolio_dashboard_example/)

* **What-if-analysis.**
<br>
Machine Learning models can run behind the dashboards, giving stakeholders the ability to interact with the model by seeing predictions according to selected variables. 
...
<iframe width="360" height="115" src="https://www.youtube.com/watch?v=22a7iOIU6sw&feature=youtu.be" frameborder="0" allowfullscreen></iframe>


Real example of data gathered from two different sources, then joined to form a unique reality on a map. 