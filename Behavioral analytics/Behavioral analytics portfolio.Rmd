...
ObjectPreprocessParams <-
  preProcess(dataset_clustering, method = c("scale"))
...
Be able to directly see which behaviour drove better or worse retention is key to define critical events, activation points or flaws in the product.
...
PlotSurvivalCI(classifier_survival_playlists_created,
  ...
  labs(title = "90 days retention by Behavioural Cohort",
       subtitle = glue::glue("In this scenario, users should be induced (by UI, reward system, etc.) to perform the activity 
                             as many times as possible to increase retention."),
...
<br>
Be able to directly see which behaviour drove better retention is key to define critical events, activation points or flaws in the product.

The use of Survival Analysis for churn outperforms standard methods, as it can draw more specific retention paths as well as being able to show confidence intervals, which add rigour to the analysis.
...
```{r Explainable ML (explanation), fig.align="center", include = FALSE, echo = FALSE, warning = FALSE, error = FALSE, message = FALSE}

# Make Object predictor

## Make dataset without dependent variable
...
ObjectFE$results %>% 
  ...
  geom_line() +
  my_minimal_theme +
  geom_vline(xintercept = 0.9, colour = "deepskyblue3", alpha = 0.3) + 
  geom_vline(xintercept = 1, colour = "deepskyblue3", alpha = 0.3) +
  annotate("rect", xmin = 0.9, xmax = 1, ymin = -Inf, ymax = Inf, fill = "deepskyblue3", alpha = .2, color = NA) + 
  annotate("text", x = 0.7, y = 0.05, label = "Disengagement\n area", size = 4, alpha = 0.7) +
  geom_segment(aes(x = 0.82, y = 0.05, xend = 0.95, yend = 0.05),
               arrow = arrow(length = unit(0.2, "cm"))) +
  labs(title = "How does usage of a feature affect engagement?",
       x = "Usage of a feature",
       y = "Change in Engagement") +
  scale_x_continuous(limits = c(0, 1), breaks = seq(0, 1, by = 0.10), labels = scales::percent_format(accuracy = 5L)) 
...
* **Critical events dashboards.**
<br>
Critical events or activation points need monitoring. Being able to constantly see the changes in user behaviour and determine if goals are met can be game changer. To see an example access [this link](https://ftoresh.shinyapps.io/portfolio_dash/)
